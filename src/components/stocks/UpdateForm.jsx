...
export default function UpdateForm() {
  ...
  useEffect(() => {
    const fetchStockData = async () => {
      try {
        const response = await fetch(`http://localhost:8080/userstocks/${stockSymbol}`);
        if (!response.ok) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/userstocks', {
        method: 'PUT',
          ...
          <div className="border-b border-gray-900/10 pb-12">
            ...
            <div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
              <div className="sm:col-span-3">
                <label htmlFor="stock_id" className="block text-sm/6 font-medium">
                  Stock ID
                </label>
                <div className="mt-2">
                  <input
